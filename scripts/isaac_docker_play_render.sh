...
OUTPUT="${1:-outputs/videos/trained_lunar_landing.mp4}"
FRAMES="${ISAAC_RENDER_FRAMES:-300}"
...
docker run --rm "${DOCKER_TTY[@]}" \
  ...
  -e ACCEPT_EULA=Y \
  -e PRIVACY_CONSENT=Y \
  -e PYTHONPATH=/workspace/LunarRocket:/workspace/isaac_rl_deps/site-packages \
  "${DOCKER_DISPLAY_ARGS[@]}" \
  ...
  "$IMAGE" \
  -lc "umask 000 && cd /isaac-sim && $CONTAINER_RUN_PREFIX ./python.sh /workspace/LunarRocket/app/isaac_play_render.py --frames '$FRAMES' --fps '$FPS' --width '$WIDTH' --height '$HEIGHT' --terrain-resolution '$TERRAIN_RESOLUTION' --output '/workspace/LunarRocket/$OUTPUT' ${ISAAC_RENDER_ARGS[*]}"

FRAME_DIR="$PROJECT_ROOT/outputs/videos/_frames_isaac_trained"
