...
OUTPUT="${1:-outputs/usd/lunar_landing_scene.usd}"
...
chmod ugo+rwx "$PROJECT_ROOT/assets/rocket" "$PROJECT_ROOT/assets/moon_heightmaps" "$PROJECT_ROOT/outputs/usd"

docker run --rm "${DOCKER_TTY[@]}" \
  --name lunar-rocket-isaac-export \
  --entrypoint /bin/bash \
  --gpus all \
  --network=host \
  --ipc=host \
  ...
  "$IMAGE" \
  -lc "umask 000 && cd /isaac-sim && ./python.sh /workspace/LunarRocket/app/main.py --headless --renderer '$RENDERER' $CAMERA_FLAG --no-render-step --steps 0 --save-stage '/workspace/LunarRocket/$OUTPUT'"
